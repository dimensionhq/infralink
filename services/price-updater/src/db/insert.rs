...
pub async fn spot_pricing_for_forecast(
    pool: &PgPool,
    region: String,
    instances: HashMap<String, Vec<SpotInstance>>,
) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (availability_zone, spot_prices) in instances.iter() {
        for spot_price in spot_prices.iter() {
            // Query the latest price for this particular combination
            let query = format!(
                "SELECT price_per_hour FROM spot_archive WHERE region = '{}' AND availability_zone = '{}' AND instance_type = '{}' ORDER BY timestamp DESC LIMIT 1",
                region, availability_zone, spot_price.instance_type
            );
